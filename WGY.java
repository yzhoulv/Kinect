...
public class WGY {
    ...
    public List<Point> getFace(String rgbPath, String depthPath, int angle) {
        ...
        int[][] faceGrid = cropFace(rgbImage, depthImage, face2Index, colorFace);
        if (angle != 0) {
    ...
    public List<Mat> creatNormals(String rgbPath, String depthPath, boolean argment) {
        List<Mat> res = new ArrayList<>();
        List<Triangle> triangles;
        List<Point> colorFace = new ArrayList<>();
        Map<Integer, Integer> face2Index = new HashMap<>();
        Mat rgbImage = loadImage(rgbPath, 0);
        Mat depthImage = loadImage(depthPath, 0);
        //noseTipCorrect(depthImage);
        int[][] faceGrid = cropFace(rgbImage, depthImage, face2Index, colorFace);
        triangles = calTri(faceGrid, face2Index);
...
//        Imgproc.resize(normImg, normImg, new Size(128, 128));
    ...
    private void noseTipCorrect(Mat image, List<Integer> lm) {
        ...
        for (int i=noseX-10; i<noseX+10; i++) {
            for (int j=noseY-10; j<noseY+10; j++) {
                double depth = image.get(j, i)[0];
    ...
    private void skinSmoothing(Mat image) {
...
//        }
    ...
    private int[][] cropFace(Mat rgbRoi, Mat depthRoi, Map<Integer, Integer> face2Index, List<Point> colorFace) {

        int[][] faceGrid = new int[112][96];
        int count = 0;
        double maxZ = -1;
        double minZ = 255;
        for (int i = 0; i < 112; i++) {
            for (int j = 0; j < 96; j++) {
                ...
                if (depthRoi.get(i, j)[0] > 10) {
                    ...
                    colorFace.add(new Point(j, -i, z, r, g, b));
                    int tmpNum = count;
                    face2Index.put(i * 96 + j, tmpNum);
                    count += 1;
                }
            }
        }
        int noseIndex = face2Index.get(68*96+48);
        ...
        for (Point tmpPoint : colorFace) {
            ...
            float z = tmpPoint.getZ()-nz; //(float) ((tmpPoint.getZ()-minZ)/(maxZ - minZ) * 255);
    ...
    private List<Triangle> calTri(int[][] faceGrid, Map<Integer, Integer> face2Index) {
        ...
        for (int i = 1; i < 112; i++) {
            for (int j = 0; j < 96 - 1; j++) {
                if (faceGrid[i][j] == -1 && faceGrid[i - 1][j + 1] == -1) {
                    if (faceGrid[i - 1][j] == -1) {
                        tris.add(new Triangle(face2Index.get(i * 96 + j), face2Index.get((i - 1) * 96 + j + 1), face2Index.get((i - 1) * 96 + j)));
                    }
                    if (faceGrid[i][j + 1] == -1) {
                        tris.add(new Triangle(face2Index.get(i * 96 + j), face2Index.get((i - 1) * 96 + j + 1), face2Index.get(i * 96 + j + 1)));
                    }
                }
            }
        }
    ...
    private Mat savaNorms(List<Point> face, List<Point> normMap, int[][] faceGrid, Map<Integer, Integer> face2Index) {
        Mat normImg = new Mat(200, 200, CvType.CV_8UC3);
        Mat depthImg = new Mat(200, 200, CvType.CV_32S);
        for (int i = 0; i < 200; i++) {
            for (int j = 0; j < 200; j++) {
                double[] num = {0, 0, 0};
                normImg.put(i, j, num);
        ...
        for (int i = 0; i < 112; i++) {
            for (int j = 0; j < 96; j++) {
                if (faceGrid[i][j] == -1) {
                    int index = face2Index.get(i * 96 + j);
                    double[] tmp = {normMap.get(index).getR(), normMap.get(index).getG(), normMap.get(index).getB()};
                    int y = Math.round(face.get(index).getX()) + 100;
                    int x = -Math.round(face.get(index).getY()) + 100;
    ...
    private Mat cropNorm(Mat normImg) {
        return new Mat(normImg, new Rect((100 - 48), (100 - 68), 96, 112));
        //return normImg;
    }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        File f = new File("./color_depth_landmark.txt");
        Scanner scan = new Scanner(f);

        String colorRoot = "H:\\3DFaceDatasets\\Lock3DFace\\color\\";
        String depthRoot = "H:\\3DFaceDatasets\\Lock3DFace\\depth\\";
        String lmRoot = "H:\\3DFaceDatasets\\Lock3DFace\\landmark\\infrared\\";
        String resRoot = "D:\\data\\project\\java\\Kinect\\Lock3d\\";
